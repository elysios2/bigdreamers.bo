...
const mapInternationalNews = (results: NewsApiResult[]): NewsItem[] => {
  ...
  results.forEach((item) => {
    if (!item.link || (!item.title && !item.description && !item.image_url))
      return;
...
export const fetchNewsInternational = async (
...
export const fetchNewsBolivia = async (
  setNewsBolivia: (data: NewsItem[]) => void,
): Promise<void> => {
